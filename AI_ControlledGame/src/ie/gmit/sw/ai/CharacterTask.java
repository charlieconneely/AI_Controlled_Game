    ...
    public Void call() throws Exception {
    	...
    	while (alive) {
        	...
        	synchronized (model) {
        		...
        		int[] direction = {rand.nextBoolean() ? 1 : -1, rand.nextBoolean() ? 1 : -1};
         		DepthLimitedSearch dls = new DepthLimitedSearch(model);
        		int[] newDirection = dls.search(temp_row, temp_col, direction, 0);
        		
        		temp_row += newDirection[0];
        		temp_col += newDirection[1];
            	...
            	if (model.isValidMove(row, col, temp_row, temp_col, enemyID)) {
            		...
            		if (model.get(temp_row, temp_col) == '1') // player 
            		{
